...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for list in symbol_quantity_list:
        atom_symbol = list[0]
        atom_weight = periodic_table_dict[atom_symbol][1]
        atom_count = list[1]
        # print(f"{atom_count} {atom_symbol} {atom_weight}")
        # Multiply the atomic mass by the quantity.
        atomic_mass = atom_weight * atom_count
...
def checkformula(formula, formula_dict):
    for formulas in formula_dict:
        if formula in formula_dict:
            formula_name = (formula_dict[formula])
        else:
...
def main():
    ...
    gram_mass = float(input("What is the mass in grams: "))
    # Call the make_periodic_table function and
    # store the periodic table in a variable.
    elements = make_periodic_table()
    molecule_name = checkformula(chemical_formula, known_molecules_dict)
    # Call the parse_formula function to convert the
    # chemical formula given by the user to a compound
    # list that stores element symbols and the quantity
    # of atoms of each element in the molecule.
    symbollist = parse_formula(chemical_formula, elements)
    # Call the compute_molar_mass function to compute the
    # molar mass of the molecule from the compound list.
    molar_mass = round(compute_molar_mass(symbollist, elements), 5)
    # Compute the number of moles in the sample.
    moles = round((gram_mass/ molar_mass), 5)
    # Print the molar mass.
    print(f"{molar_mass} grams/moles")
